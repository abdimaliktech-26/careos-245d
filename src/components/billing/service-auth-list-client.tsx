          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {auths.map((a) => {
                ...
                return (
                  <tr key={a.id as string} className="text-sm hover:bg-gray-50/80 transition-colors group">
                    <td className="px-5 py-3 font-medium text-[#3A2A4A]">
                      {(a.clients as { legal_name?: string } | null)?.legal_name ?? '—'}
                    </td>
                    <td className="px-5 py-3 font-mono text-xs">{a.auth_number as string}</td>
                    <td className="px-5 py-3 text-gray-500">{a.payer as string}</td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="rounded-lg border border-gray-200 px-2.5 py-1 text-[10px] font-semibold text-gray-500 hover:bg-gray-50 transition-colors"
                        >
                          Edit
                        </button>
                        <Link
                          href={`/billing-readiness/claims?search=${a.auth_number as string}`}
                          className="rounded-lg border border-gray-200 px-2.5 py-1 text-[10px] font-semibold text-[#E8799E] hover:bg-[#EEF2FF] transition-colors"
                        >
                          Claims
                        </Link>
                      </div>
