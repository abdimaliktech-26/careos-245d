    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-gray-100 bg-white px-4 py-3 lg:hidden shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-bold text-[#3A2A4A]">{branding?.name ?? 'CareIntake'}</span>
        </div>
        <button
          onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div className="absolute left-0 top-0 bottom-0 w-[260px] bg-white shadow-xl animate-in-slide-left">
            {children}
          </div>
        </div>
      )}
