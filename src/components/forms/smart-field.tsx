...
export function SmartField({ field, value, onChange, carryForward, onApplyCarryForward }: SmartFieldProps) {
  ...
  const renderField = () => {
    ...
    if (field.type === 'section_header') {
      return <h4 className="text-sm font-semibold text-gray-800">{field.label}</h4>
    }
    return null
  }
  ...
  return (
    <div className="relative">
      ...
      {showIndicator && (
        <div className="mt-1.5 flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 text-[11px] font-medium text-[#E8799E] hover:text-[#C06080] transition-colors"
          >
      ...
      {showPrevious && carryForward && (
        <div className="mt-1.5 rounded-lg border border-[#E8799E]/20 bg-[#EEF2FF]/50 px-3 py-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-[11px] font-medium text-[#64748B] shrink-0">Previously:</span>
              <span className="text-[12px] text-[#3A2A4A] truncate">
            ...
            {onApplyCarryForward && (
              <button
                ...
                className="shrink-0 rounded-lg bg-[#E8799E] px-2.5 py-1 text-[10px] font-semibold text-white hover:bg-[#C06080] transition-colors"
              >
                Apply
              </button>
            )}
