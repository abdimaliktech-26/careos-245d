...
export function ContactField({ field, value, onChange }: ContactFieldProps) {
  ...
  return (
    <fieldset className="border border-border rounded-md p-3 space-y-2">
      <legend className="text-sm font-medium text-foreground px-1">{field.label}</legend>
      <input type="text" placeholder="Name" value={value.name} onChange={update('name')}
        className="w-full border border-border rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
