...
export function IncidentTimeline({ incident }: { incident: Record<string, unknown> }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
      <h2 className="text-xs font-semibold tracking-[0.08em] text-gray-400 uppercase mb-4">Timeline</h2>
      <div className="space-y-0">
        {visible.map((event, i) => (
          <div key={event.label} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#E8799E]/10 text-[10px] font-bold text-[#E8799E]">
                {event.icon}
              </div>
              {i < visible.length - 1 && <div className="w-px flex-1 bg-gray-100" />}
            </div>
            <div className="pb-4">
              <p className="text-sm font-semibold text-[#3A2A4A]">{event.label}</p>
              <p className="text-xs text-gray-500">{event.date ? new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}</p>
            </div>
          </div>
        ))}
      </div>
