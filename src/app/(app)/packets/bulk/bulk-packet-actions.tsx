...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    ...
    needs_signature: 'bg-amber-50 text-amber-700 border-amber-200',
    overdue: 'bg-red-50 text-red-700 border-red-200',
    not_started: 'bg-gray-100 text-gray-600 border-gray-200',
  }
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold border ${colors[status] ?? 'bg-gray-100 text-gray-600'}`}>
      {status.replace(/_/g, ' ')}
    </span>
  )
}
    ...
    <div className="space-y-4">
      ...
      {result && (
        <div className="rounded-2xl border p-4 bg-gray-50 text-sm space-y-1">
          {result.success > 0 && <p className="text-emerald-600 font-semibold">{result.success} packets updated.</p>}
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setShowConfirm(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-[#E8799E] mb-4">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 11.08V12a10 10 0 11-5.93-9.14" />
                <polyline points="22 4 12 14.01 9 11.01" />
              </svg>
            </div>
            <p className="text-sm font-bold text-[#3A2A4A] mb-1">Confirm Bulk Action</p>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                className="rounded-lg bg-[#E8799E] px-4 py-2 text-xs font-semibold text-white hover:opacity-90"
              >
                Confirm
              </button>
            </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 text-left">
              ...
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400">Type</th>
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400">Status</th>
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400">Due</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {packets.map(p => (
              <tr key={p.id} className="hover:bg-gray-50/60 transition-colors">
                ...
                <td className="px-4 py-3 font-medium text-[#3A2A4A]">{p.client_name}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{p.packet_type.replace(/_/g, ' ')}</td>
