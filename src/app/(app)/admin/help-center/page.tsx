    ...
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage knowledge base articles for your organization.</p>
        </div>
        <Link
      ...
      {articles.length === 0 ? (
        <div className="rounded-2xl border border-border bg-card px-6 py-12 text-center" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <p className="text-sm text-muted-foreground">No articles yet.</p>
          <Link href="/admin/help-center/new" className="text-sm text-primary hover:underline mt-2 inline-block">
              ...
              {articles.map((article) => {
                ...
                return (
                  <tr key={article.id} className="border-b border-gray-50">
                    ...
                    <td className="px-6 py-4 text-xs text-muted-foreground">{updated}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/admin/help-center/${article.id}/edit`}
                          className="rounded-lg border border-border px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:bg-muted/40 transition-colors inline-flex items-center gap-1.5"
                        >
                          <EditIcon />
                          Edit
                        </Link>
