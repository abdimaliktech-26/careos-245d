  ...
  return (
    <div className="max-w-4xl mx-auto p-8 space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#3A2A4A]">Health Conditions — {client.legal_name}</h1>
      </div>
      ...
      <section className="rounded-2xl border border-gray-100 bg-white" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-[#3A2A4A]">All Conditions ({conditions.length})</h2>
        </div>
        {conditions.length === 0 ? (
              ...
              <div key={cond.id} className="px-6 py-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {cond.severity && (
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${SEVERITY_COLORS[cond.severity] ?? 'bg-gray-100 text-gray-600'}`}>
                        {cond.severity}
                      </span>
                    )}
                  </div>
                </div>
                <div className="mt-1 flex gap-4 text-xs text-[#64748B]">
                ...
                {cond.notes && (
                  <p className="text-xs text-[#64748B] mt-1 italic">{cond.notes}</p>
                )}
              </div>
            ))}
          </div>
